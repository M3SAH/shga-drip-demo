...
const VIEWS = {
  ...
  manage:    { title: "Manage Products", crumb: "Edit or delete products" },
};
...
function buildTable(items) {
  const rows = items.map(p => {
    ...
    const sizes  = (p.sizes  || []).map(s => `<span class="tag">${esc(s)}</span>`).join("") || `<span style="color:var(--dim)">—</span>`;
    const colors = (p.colors || []).map(c => `<span class="tag">${esc(c)}</span>`).join("") || `<span style="color:var(--dim)">—</span>`;
    ...
    return `<tr>
      ...
      <td>${badge}</td>
      <td><div class="tag-wrap">${sizes}</div></td>
      <td><div class="tag-wrap">${colors}</div></td>
      <td>
  ...
  return `<table>
    <thead><tr>
      ...
      <th>Stock</th><th>Sizes</th><th>Colors</th><th>Actions</th>
    </tr></thead>
    <tbody>${rows}</tbody>
  </table>`;
}
...
const GRADE_ORDER = ["Standard", "Premium", "Pro", "Luxury"];
