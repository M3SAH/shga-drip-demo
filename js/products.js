...
function toProduct(docSnap) {
  ...
  return {
    ...
    sizes:       Array.isArray(d.sizes)  ? d.sizes  : ["M","L","XL","XXL"],
    colors:      Array.isArray(d.colors) ? d.colors : ["White","Black"],
    image:       resolvedImageUrl,
    imageUrl:    resolvedImageUrl,
    type:        d.category === "Sleeveless" ? "sleeveless" : "design",
  };
}
...
function start() {
  ...
  onSnapshot(q, snapshot => {
    ...
    const fingerprint = JSON.stringify(fresh.map(p => ({ id: p.id, stock: p.stock, name: p.name })));
